...
if(isset($_POST['add'])){
	if($_POST['pup']=='proj'){
		...
		if(!empty($_POST['deadline'])){
			...
			$deadline = $project->addChild('deadline',date('Y-m-d'));
		}
		$dom = new DomDocument();
		...
		if($_POST['user']!=''){
			...
			$_SESSION['message'] = '<p class="padded b-s-t" style="color: #ff0000;"><strong>Role not created.</strong><br />A user must be selected.</p>';
			header('location: page_edit_roles.php');
		}
	} else {
		...
		if($_POST['title']!=''){$task->addAttribute('title', $_POST['title']);}else{$task->addAttribute('title', '[untitled task #'.$_POST['id'].']');}
		if($_POST['user']!=''){$task->addAttribute('owner', $_POST['user']);}else{$task->addAttribute('owner','');}
		if($_POST['project']!=''){$task->addAttribute('project', $_POST['project']);}else{$task->addAttribute('project','');}
