...
if(isset($_POST['edit'])){
	if($_POST['pup']=='projects'){
		...
		foreach($tasks->task as $task){
			if($task['id'] == $_POST['id']){
				if($_POST['owner']!=''){$task['owner'] = $_POST['owner'];}else{$task['owner']='n/a';}
				if($_POST['title']!=''){$task['title'] = $_POST['title'];}else{$task['title']='[untitled task #'.$_POST['id'].']';}
				if($_POST['project']){$task['project'] = $_POST['project'];}else{$task['project']='n/a';}
