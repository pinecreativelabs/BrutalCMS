...
function loginUser($user,$pass){
	...
	// Check user existance	
	/*
	$pfile = fopen("molds/pad/module/data/users.csv","r");
	rewind($pfile);
    while (!feof($pfile)) {
        $line = fgets($pfile);
        $tmp = explode(',', $line);
        if ($tmp[0] == $user) {
			...
			} elseif(trim($tmp[1]) == trim(sha1($pass))){
				$validUser= true;
            	$_SESSION['userName'] = $user;
			} else {} break;
        }
	...
	*/
	$pfile = simplexml_load_file('molds/pad/module/data/users.xml');
	foreach($pfile->user as $account){
		if ($account['username'] == $user) {
            // User exists, check password
            if (trim($account->password) == trim(md5($pass))){
            	$validUser= true;
            	$_SESSION['userName'] = $user;
			} elseif(trim($account->password) == trim(sha1($pass))){
				$validUser= true;
            	$_SESSION['userName'] = $user;
			} else {} break;
        }
