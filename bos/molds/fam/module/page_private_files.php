...
if($fam_cm=='relative'){
$miscdir = 'app/users/'.$current_user.'/files/';
} else {$miscdir = $bosdir.'app/users/'.$current_user.'/files/';}

$miscpath = $pdir.'/app/users/'.$current_user.'/files/';
$trashpath = $pdir.'/app/users/'.$current_user.'/trash/';
			...
			if ($opendirectory = opendir($miscpath)){
				...
				foreach($files as $file) {
					if(is_file($miscpath.$file)){
						...
						echo "<div class=\"del sepia-h\" data-fid=\"".$fid."\"><img src=\"../../../core/css/images/trash.png\" class=\"tiny-thumb\" id=\"fid_thumb_misc_".$fid."\"/></div></div>";
					}
				}
			}
		}
	...
	<div class="container-960">
		...
		<div class="unset-bg padded">
			<div class="fu padded terminal b-s-t">
				<input class="FU" type="file" path="../../../app/users/<?php echo $current_user;?>/files/" startOn="manually" buttonCaption="Upload" buttonClass="fu-fb" multi="true" afterUpload="link" maxSize="<?php echo $maxFileSize; ?>" />
			</div>
		</div>
	</div>
</div>
...
<div id="help" style="display:none;">
	<div class="padded rounded b-s-k lime-b depixel" style="background: #000; max-width: 600px;">
		...
		<p><img src="../../../core/css/images/trash.png" alt="move" class="micro-thumb"/> <small>[Moves file to <em>My Trash</em> folder.]</small></p>
	</div>
</div>
	...
	$('.fmisc .del').click(function(){
		...
		$.ajax({
			...
			success: function(response){
				if(response == 1){
					$("#fid_thumb_misc_" + id).attr("src","../../../core/css/images/deleted.png");
				} else { $("#fid_thumb_misc_" + id).attr("src","../../../core/css/images/error.jpg");}
			}
		});
	});
});
