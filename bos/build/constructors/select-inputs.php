...
$country_select_profile = '<select name="country">'.PHP_EOL;
...
if (($ccc_handle = fopen($get_ccc_data, "r")) !== FALSE) {
	while (($cccdata = fgetcsv($ccc_handle, 1000, ",")) !== FALSE) {
		...
		if (in_array($row, $skip_row_number)){continue;} else {
			...
			if($up_country==$cccdata[0]){
				$country_select_profile.='<option value="'.$cccdata[0].'" selected>'.$cccdata[0].'</option>'.PHP_EOL;
			} else {$country_select_profile.='<option value="'.$cccdata[0].'">'.$cccdata[0].'</option>'.PHP_EOL;}
...
$country_select_profile .= '</select>'.PHP_EOL;
...
$timezone_select_profile = '<select name="timezone">'.PHP_EOL;
$row=0;
$skip_row_number = array("1");
if (($tz_handle = fopen($get_tz_data, "r")) !== FALSE) {
	while (($tzdata = fgetcsv($tz_handle, 1000, ",")) !== FALSE) {
		...
		if (in_array($row, $skip_row_number)){continue;} else {
			...
			if($up_tz==$tzdata[0]){
				$timezone_select_profile.='<option value="'.$tzdata[0].'" selected>'.$tzdata[0].'</option>'.PHP_EOL;
			}else{$timezone_select_profile.='<option value="'.$tzdata[0].'">'.$tzdata[0].'</option>'.PHP_EOL;}
...
$timezone_select_profile.='</select>'.PHP_EOL;
...
$language_select_profile = '<select name="language">'.PHP_EOL;
$row=0;
$skip_row_number = array("1");
if (($lang_handle = fopen($get_lang_data, "r")) !== FALSE) {
	while (($langdata = fgetcsv($lang_handle, 1000, ",")) !== FALSE) {
		...
		if (in_array($row, $skip_row_number)){continue;} else {
			...
			}else{$language_select_user.='<option value="'.$langdata[1].'">'.$langdata[2].'</option>'.PHP_EOL;}
			if($up_lang==$langdata[1]){
				$language_select_profile.='<option value="'.$langdata[1].'" selected>'.$langdata[2].'</option>'.PHP_EOL;
			}else{$language_select_profile.='<option value="'.$langdata[1].'">'.$langdata[2].'</option>'.PHP_EOL;}
			if($language==$langdata[1]){
				$language_select.='<option value="'.$langdata[1].'" selected>'.$langdata[2].'</option>'.PHP_EOL;
			}else{$language_select.='<option value="'.$langdata[1].'">'.$langdata[2].'</option>'.PHP_EOL;}
		}
	} fclose($lang_handle);	
}
$language_select.='</select>'.PHP_EOL;
$language_select_user.='</select>'.PHP_EOL;
$language_select_profile.='</select>'.PHP_EOL;
?>