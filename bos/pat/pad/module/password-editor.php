...
function makeTableRow($lineCnt, $row, $columns) {
    ...
    for ($columnCnt=0; $columnCnt<$columns; $columnCnt++) {
        $h .= "<td><input class=\"form-control" . ($columnCnt==0 ? " input-col-first" : " input-col-rest") . "\"".($row[$columnCnt]==$_SESSION['userName'] ? "style=\"background: red;\"" : "")."rel=\"input-" . $lineCnt . "\" disabled type=\"text\" value=\"" . $row[$columnCnt] . "\"></td>";
    } 
	$h .= "<td>";
